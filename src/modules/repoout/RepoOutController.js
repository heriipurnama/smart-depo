...
class RepoOutController {
	...
	static async insertRepoOut(req, res, next) {
		let {
			voyno,
			vesid,
			retype,
	...
	static async viewDataRepoOutDetails(req, res, next) {
		try {
			let data = await container_process.sequelize.query(
	...
	static async insertPraRepoOutDetail(req, res, next) {
		let {
			crno,
			cccode,
			cpiorderno,
	...
	static async updateDataRepoOutDetails(req, res, next) {
