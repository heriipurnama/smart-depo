...
class dataListReportController {
	static async listGateOut(req, res, next) {
		...
		let limits = limit !== null ? limit : 9;
		let offsets = offset !== null ? offset : 0;
		...
		try {
			...
			// note : coba bikin count array datas hasil raw query, dengan JS
	...
	static async listSurvey(req, res, next) {
		try {
			let datas = await container_process.sequelize.query(
				`SELECT SVY.SVID,CON.CRNO,PR.PRCODE,CASE WHEN DATE_FORMAT(CP.CPITGL,'%d/%m/%Y')='00/00/0000' THEN '' ELSE DATE_FORMAT(CP.CPITGL,'%d/%m/%Y') END AS CPITGL,DATE_FORMAT(SVY.SVSURDAT,'%d/%m/%Y') as SVSURDAT,SVY.SVCOND FROM
			...
			baseResponse({ message: "List Survey", data: { datas } })(res, 200);
		} catch (error) {
			res.status(403);
			next(error);
		}
	...
	static async listEstimation(req, res, next) {
		try {
			let datas = await container_process.sequelize.query(
                ...
                FROM 
                 coins_container_process cp INNER JOIN tblcontainer con ON
                  cp.CRNO = con.CRNO
                  INNER JOIN coins_survey surv ON
                  surv.CPID = cp.CPID
                  INNER JOIN coins_repair rp ON
                  rp.SVID = surv.SVID
                  LEFT JOINtblprincipal pr ON
		...
		try {
			let datas = await container_process.sequelize.query(
               ...
               LEFT JOIN tblprincipal pr ON
                 pr.PRCODE = cp.CPOPR
               Where surv.TYPE='1' AND con.CRLASTACT in('WA','WW','IW','RP','CR','OW','CP','WR') and rp.RPFINALEST='1' 
			...
			baseResponse({ message: "List Approvals", data: { datas } })(res, 200);
		} catch (error) {
			res.status(403);
			next(error);
		}
	...
	static async listWO(req, res, next) {
		try {
			let datas = await container_process.sequelize.query(
               ...
               ORDER BY WO.WODATE desc,WO.WONO desc`,
				{
					type: container_process.SELECT,
				}
			);

			baseResponse({ message: "List WO", data: { datas } })(res, 200);
		} catch (error) {
			res.status(403);
			next(error);
		}
	...
	static async listMnR(req, res, next) {
		try {
			let datas = await container_process.sequelize.query(
				`SELECT tblcontainer.CRNO,container_repair.SVID,container_repair.RPTGLAPPVPR,container_repair.RPVER,tblcontainer_code.CTCODE,tblcontainer_code.CCLENGTH,tblcontainer_code.CCHEIGHT,container_process.CPITGL,container_survey.SVCOND FROM 
                    ...
                    LEFT JOIN tblcontainer_code ON tblcontainer.CCCODE = tblcontainer_code.CCCODE
                     WHERE tblcontainer.CRLASTACT='WW' AND container_process.CPOPR='".trim($_GET["PRCODE"])."' AND
                      container_repair.RPFINALEST='1' AND container_survey.TYPE='1' ORDER BY container_repair.RPTGLAPPVPR desc,tblcontainer.CRNO`,
				{
					type: container_process.SELECT,
				}
			);
	...
	static async listRepoIn(req, res, next) {
     ...
     WHERE $type like 'RI%'`,
			...
			// Note :
			// tabel container_repo belum ada di db! dan tanya variabel $type!
			baseResponse({ message: "List WO", data: { datas } })(res, 200);
		} catch (error) {
			res.status(403);
			next(error);
